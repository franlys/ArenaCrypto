...
export default function DragonTowerPage() {
  ...
  const [tileStates, setTileStates] = useState<TileState[][]>([]); // [level][tile]
  ...
  const initBoard = (diff: Difficulty) => {
    const tiles = DIFFICULTY_CONFIG[diff].tiles;
    return Array.from({ length: MAX_LEVELS }, () => Array(tiles).fill("hidden") as TileState[]);
  };

  const startGame = async () => {
    setLoading(true); setMsg(""); setMineAt(null);
    const data = await apiCall({ action: "start", amount, difficulty });
    setLoading(false);
    ...
    <div className={styles.page}>
      <div className={styles.header}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
          ...
          {isTestUser && (
            <span className={styles.testBadge}>MODO PRUEBA</span>
          )}
        </div>
        <p className={styles.subtitle}>
          Sube cada nivel eligiendo una puerta. Cada acierto multiplica. Un error y caes.
          {isTestUser && <span style={{ color: "rgba(167,139,250,0.7)", marginLeft: "0.5rem" }}>· Usando saldo de prueba</span>}
        </p>
      </div>
      ...
      <div className={styles.layout}>
        ...
        <div className={styles.controls}>
          <div className={styles.controlGroup}>
            <label className={styles.label}>APUESTA {isTestUser ? "(SALDO PRUEBA)" : "(USDC)"}</label>
            <div className={styles.amountRow}>
              <input type="number" className={styles.input} value={amount}
                onChange={e => setAmount(Number(e.target.value))} disabled={!isIdle} />
              {[5, 10, 25, 50].map(v => (
                <button key={v} className={styles.pill} onClick={() => setAmount(v)} disabled={!isIdle}>${v}</button>
              ))}
            </div>
          </div>
          ...
          <div className={styles.statsPanel}>
            <div className={styles.stat}>
              ...
              <span className={styles.statValue}>{currentLevel} / {MAX_LEVELS}</span>
            </div>
            <div className={styles.stat}>
              <span className={styles.statLabel}>MULTIPLICADOR</span>
              <span className={styles.statValue} style={{ color: "hsl(270,80%,70%)" }}>{multiplier.toFixed(2)}x</span>
            </div>
            <div className={styles.stat}>
              <span className={styles.statLabel}>GANANCIA POTENCIAL</span>
              <span className={styles.statValue} style={{ color: "#22c55e" }}>${(amount * multiplier).toFixed(2)}</span>
            </div>
          </div>
        ...
        <div className={styles.tower}>
          {Array.from({ length: MAX_LEVELS }, (_, i) => {
            const levelNum = MAX_LEVELS - i;         // 9 → 1 top to bottom
            const boardIdx = i;                       // 0 = top row
