...
function dropPlinko(serverSeed: string, rows: number, amount: number): { path: string[]; slot: number } {
  ...
  // DYNAMIC DIFFICULTY: If bet is high (> $100), we increase the pull towards center
  const centerBias = amount > 100 ? 0.05 : 0; // 5% more likely to stay center for high bets
